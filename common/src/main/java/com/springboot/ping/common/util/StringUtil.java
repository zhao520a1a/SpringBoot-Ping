...
public class StringUtil {
	...
	private static String replaceAllSpecialCharacter(String value){
	 ...
	 * @return
	 */
	public static String formatData(Object obj,Format type){
		if(obj==null){
			return null;
		}
		switch (type) {
		case NUMBER://格式化数字，逗号隔开千分位10,000.00
			obj = MathUtil.roundFMT(replaceAllSpecialCharacter(obj.toString()));
			break;
		case AMT://格式化金额，逗号隔开千分位，￥开头 ￥10,000.00
			obj = "￥"+MathUtil.roundFMT(replaceAllSpecialCharacter(obj.toString()));
			break;
		case RATE://格式化百分比类，%结尾 50.5%
			String data = replaceAllSpecialCharacter(obj.toString());
			if(data.trim().startsWith(".")){
				data = "0"+data.trim();
			}
			obj = data+"%";
			break;
		case DATE://日期,格式化成YYYY-MM-DD,比如2015-07-09
			if(obj instanceof Date){
				obj = DateUtil.formatDate((Date)obj, "yyyy-MM-dd");
			}
			break;
		case DATETIME://日期+时间,格式化成YYYY-MM-DD HH:mm:ss,比如2015-07-09 12:12:12
			if(obj instanceof Date){
				obj = DateUtil.formatDate((Date)obj, "yyyy-MM-dd HH:mm:ss");
			}
			break;
		case TIME://时间,格式化成HH:mm:ss,比如12:12:12
			if(obj instanceof Date){
				obj = DateUtil.formatDate((Date)obj, "HH:mm:ss");
			}
			break;
		default:
			if(obj instanceof Date){
