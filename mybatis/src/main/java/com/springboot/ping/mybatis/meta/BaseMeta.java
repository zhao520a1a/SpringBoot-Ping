package com.springboot.ping.mybatis.meta;
...
import com.springboot.ping.mybatis.enums.DbDialect;
...
public class BaseMeta {
	...
	public static void init(MybatisProperties mybatisProperties, DataSourceProperties dataSourceProperties){
        ...
        if(jdbcUrl.toUpperCase().indexOf(DbDialect.MYSQL.toString())>=0){
            ...
            throw new RuntimeException("jdbcUrl不是"+StringUtil.parseArray(DbDialect.values())+"中的数据库系统！！！");
        }
        logger.info("初始化项目实体元数据 start...");
        Set<Class> clazzs = doScan(mybatisProperties.getBasePackages());
	...
	private static Set<Class> doScan(Set<String> basePackages) {
            ...
            for (String bp : basePackages) {
                ...
                for (Resource resource : rs) {
                    ...
                    try {
                    	...
                    	logger.debug(StringUtil.getTrace(e));
                    }
                }
			}
    ...
    public static void main(String[] args) {
        System.out.println(StringUtil.parseArray(DbDialect.values()));
    }
}
