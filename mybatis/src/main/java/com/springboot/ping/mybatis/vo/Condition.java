...
public class Condition implements Serializable{
	...
	private Object convertValue(DbType dbType,Operator operator,Object value) {
		...
		case LIKE:
		case NOTLIKE:
			tempValue = "%"+((value instanceof Date)?DateUtil.formatDate((Date)value, DEFAULT_DATE_PATTERN):value.toString())+"%";
			break;
		case IN:
		case NOTIN:
		...
		default:
			...
			case FLOAT:
				...
				break;
			case DATE:
				tempValue = (value instanceof Date)?DateUtil.formatDate((Date)value, DEFAULT_DATETIME_PATTERN):value;
				break;
			default:
				break;
			}
	...
	public static Condition getCondition(String column_dbtype_operator,Object value,Class<?> entityClass){
		if(!StringUtil.isNotEmpty(column_dbtype_operator)){
			return new Condition();
		}
		column_dbtype_operator = column_dbtype_operator.trim();
		if(!Pattern.matches(CONDITION_PATTERN, column_dbtype_operator)){
			log.warn("传入的参数值["+column_dbtype_operator+"]格式不正确!正确格式为:字段名称(只能数字、字母或者下划线)、数据库类型"+StringUtil.parseArray(DbType.values())+"和sql操作符"+StringUtil.parseArray(Operator.values())+"，以'"+CONDITION_SEPARATOR+"'分隔!");
			return new Condition();
		}
